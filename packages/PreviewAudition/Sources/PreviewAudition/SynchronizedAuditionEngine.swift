...
public enum AuditionEngineError: Error, CustomStringConvertible {
    case noAudio
...
/// Runs every version simultaneously through one engine. A/B selection changes only
/// player gains, so every switch retains the same sample position.
@MainActor
public final class SynchronizedAuditionEngine {
    ...
    public func load(urls: [URL]) async throws {
        unload()
        ...
        let loaded = try await Task.detached(priority: .userInitiated) {
            ...
            for url in urls {
                ...
                formats.append(file.processingFormat)
            }
            return LoadedAuditionAudio(buffers: buffers, formats: formats)
        }.value
        engine = AVAudioEngine()
        for (buffer, format) in zip(loaded.buffers, loaded.formats) {
    ...
    public func pause() { for player in players { player.pause() } }
...
private struct LoadedAuditionAudio: @unchecked Sendable {
    var buffers: [AVAudioPCMBuffer]
    var formats: [AVAudioFormat]
}
