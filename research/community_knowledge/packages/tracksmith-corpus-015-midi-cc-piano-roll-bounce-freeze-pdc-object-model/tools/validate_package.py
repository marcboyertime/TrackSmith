...
def load_prior(path):
 ...
 for root in roots:
  ...
  if pid=='tracksmith-corpus-015-midi-cc-piano-roll-bounce-freeze-pdc-object-model': continue
...
def validate_root(root,prior=None,target=None,full=False):
 ...
 if manifest.get('id_namespace')!='pkg015' or manifest.get('package_number')!=15: fail('namespace/sequence mismatch')
 if integ.get('namespace')!='pkg015' or integ.get('sequence')!=15 or integ.get('package_id')!=manifest.get('package_id'): fail('integration manifest identity mismatch')
 ...
 for name,rows in files.items():
  ...
  if any(not x.startswith('pkg015.') for x in ids): fail(f'ID outside pkg015 in {name}')
