...
def main() -> None:
    ...
    if manifest.get("id_namespace") != "pkg006" or manifest.get("package_number") != 6:
    ...
    for name, rows in files.items():
        ...
        if any(not identifier.startswith("pkg006.") for identifier in ids):
            fail(f"ID outside pkg006 namespace in {name}")
    ...
    if domain_counts != Counter({"saturation_harmonic_distortion": 204, "transient_shaping": 180}):
    ...
    for query in ("saturation", "aliasing oversampling", "kick clipping", "transient compressor", "Enveloper attack"):
