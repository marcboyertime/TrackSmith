...
def main() -> None:
    ...
    if manifest.get("id_namespace") != "pkg008" or manifest.get("package_number") != 8:
    ...
    for name, rows in files.items():
        ...
        if any(not identifier.startswith("pkg008.") for identifier in ids):
            fail(f"ID outside pkg008 namespace in {name}")
    ...
    if domain_counts != Counter({"editing": 220, "layering": 200}):
    ...
    for query in ("vocal breath cleanup", "region boundary crossfade", "align vocal doubles", "piano pedal noise", "phase locked drum editing", "too many layers smaller", "kick transient body sub", "clean sub distorted bass", "layer contribution texture width atmosphere", "Summing Stack Folder Stack"):
