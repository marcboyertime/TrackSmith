...
def load_prior(path):
 ...
 for root in roots:
  ...
  if pid=='tracksmith-corpus-010-flex-time-manual-timing': continue
...
def validate_root(root,prior=None,target=None,full=False):
 ...
 if manifest.get('id_namespace')!='pkg010' or manifest.get('package_number')!=10: fail('namespace/sequence mismatch')
 if integ.get('namespace')!='pkg010' or integ.get('sequence')!=10 or integ.get('package_id')!=manifest.get('package_id'): fail('integration manifest identity mismatch')
 ...
 for name,rows in files.items():
  ...
  if any(not x.startswith('pkg010.') for x in ids): fail(f'ID outside pkg010 in {name}')
