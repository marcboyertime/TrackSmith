...
def load_prior(path):
 ...
 for root in roots:
  ...
  if pid=='tracksmith-corpus-012-recording-latency-monitoring-comping-punch': continue
...
def validate_root(root,prior=None,target=None,full=False):
 ...
 if manifest.get('id_namespace')!='pkg012' or manifest.get('package_number')!=12: fail('namespace/sequence mismatch')
 if integ.get('namespace')!='pkg012' or integ.get('sequence')!=12 or integ.get('package_id')!=manifest.get('package_id'): fail('integration manifest identity mismatch')
 ...
 for name,rows in files.items():
  ...
  if any(not x.startswith('pkg012.') for x in ids): fail(f'ID outside pkg012 in {name}')
