...
def main() -> None:
    ...
    if manifest.get("id_namespace") != "pkg007" or manifest.get("package_number") != 7:
    ...
    for name, rows in files.items():
        ...
        if any(not identifier.startswith("pkg007.") for identifier in ids):
            fail(f"ID outside pkg007 namespace in {name}")
    ...
    if domain_counts != Counter({"phase_polarity": 144, "stereo_imaging": 132, "panning": 120}):
    ...
    for query in ("phase polarity", "negative correlation", "Stereo Pan Balance", "wide mix mono", "send pan return pan", "Sample Delay alignment"):
