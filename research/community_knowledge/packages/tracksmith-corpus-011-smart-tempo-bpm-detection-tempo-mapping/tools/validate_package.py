...
def load_prior(path):
 ...
 for root in roots:
  ...
  if pid=='tracksmith-corpus-011-smart-tempo-bpm-detection-tempo-mapping': continue
...
def validate_root(root,prior=None,target=None,full=False):
 ...
 if manifest.get('id_namespace')!='pkg011' or manifest.get('package_number')!=11: fail('namespace/sequence mismatch')
 if integ.get('namespace')!='pkg011' or integ.get('sequence')!=11 or integ.get('package_id')!=manifest.get('package_id'): fail('integration manifest identity mismatch')
 ...
 for name,rows in files.items():
  ...
  if any(not x.startswith('pkg011.') for x in ids): fail(f'ID outside pkg010 in {name}')
