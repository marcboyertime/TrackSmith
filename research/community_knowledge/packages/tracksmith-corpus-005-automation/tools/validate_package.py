...
def main() -> None:
    ...
    if manifest.get("id_namespace") != "pkg005" or manifest.get("package_number") != 5:
    ...
    for name, rows in files.items():
        ...
        if any(not identifier.startswith("pkg005.") for identifier in ids):
            fail(f"ID outside pkg005 namespace in {name}")
    ...
    for query in ("fader snaps back", "Touch Latch", "delay throw send", "track region automation", "automation late"):
