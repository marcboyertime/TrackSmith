...
def load_prior(path):
 ...
 for root in roots:
  ...
  if pid=='tracksmith-corpus-013-sends-buses-auxes-track-stacks-groups-submixes': continue
...
def validate_root(root,prior=None,target=None,full=False):
 ...
 if manifest.get('id_namespace')!='pkg013' or manifest.get('package_number')!=13: fail('namespace/sequence mismatch')
 if integ.get('namespace')!='pkg013' or integ.get('sequence')!=13 or integ.get('package_id')!=manifest.get('package_id'): fail('integration manifest identity mismatch')
 ...
 for name,rows in files.items():
  ...
  if any(not x.startswith('pkg013.') for x in ids): fail(f'ID outside pkg013 in {name}')
