...
def live_cloud_evidence():
 """Fail closed on the two sanitized, evaluation-only cloud artifacts.
 ...
 no_tool=load(LIVE_NO_TOOL,'no_tool');full_tool=load(LIVE_FULL_TOOL,'full_tool');full_artifact_sha=sh(LIVE_FULL_TOOL.read_bytes())
 no_source=base(no_tool,'no_tool','historical_p17_no_tool_context_source_vocal_plus_level_only');full_source=base(full_tool,'full_tool','deterministic injected read-only context')
 def source_valid(source,allow_known_pre_fix_unavailable=False):
  ...
  if re.fullmatch(r'[0-9a-f]{64}',str(patch)):return 'captured'
  if allow_known_pre_fix_unavailable and patch=='unavailable' and full_artifact_sha==KNOWN_PRE_FIX_FULL_TOOL_SHA256 and source.get('commit')==no_source.get('commit') and source.get('index_tree')==no_source.get('index_tree'):return 'known_pre_fix_pipe_deadlock_unavailable'
  raise ValueError('P19 evidence worktree provenance unavailable outside the known pre-fix full-tool artifact')
 no_provenance=source_valid(no_source);full_provenance=source_valid(full_source,True)
 ...
 if no_tool.get('counts')!=no_counts or full_tool.get('counts')!=full_counts:raise ValueError('P19 live cloud count mismatch')
 ...
 def honest_full_tool():
  ...
  return cost
 no_cost=successful_no_tool();full_cost=honest_full_tool()
 for path in (LIVE_NO_TOOL,LIVE_FULL_TOOL):
 ...
 def summary(path,evidence,status,cost,provenance,requests,completed,judgments,tools):
  return {'status':status,'artifact_sha256':sh(path.read_bytes()),'artifact_bytes':path.stat().st_size,'schema_version':evidence['schema_version'],'lane':evidence['lane'],'generation_requests':requests,'generation_samples':36,'completed_generations':completed,'valid_supporting_judgments':judgments,'tools_enabled_samples':tools,'cost':{'cap_microusd':cost['cap_microusd'],'external_unknown_hold_microusd':cost['external_unknown_hold_microusd'],'reserved_microusd':cost['reserved_microusd'],'confirmed_p19_spent_microusd':cost['spent_microusd'],'poisoned':cost['poisoned']},'worktree_provenance':provenance,'runtime_resource_exclusion':'passed_artifacts_outside_live_resource_roots'}
 return {'no_tool':summary(LIVE_NO_TOOL,no_tool,'accepted_completed_no_tool',no_cost,no_provenance,48,36,12,0),'full_tool':summary(LIVE_FULL_TOOL,full_tool,'accepted_honest_partial_full_tool',full_cost,full_provenance,142,26,5,36)}
...
ARTIFACT_PATHS={'suite':SUITE,'manifest':MANIFEST,'candidate_index':INDEX,'candidate_manifest':IM,'policy':POLICY,'tool_diagnostic':E/'PACKAGE_019_TOOL_DIAGNOSTIC.json','live_no_tool_evidence':LIVE_NO_TOOL,'live_full_tool_evidence':LIVE_FULL_TOOL}
REQUIRED_VALIDATION_KEYS={'suite_check','forbidden_runtime_resource_scan','retrieval_mirror_measurement','swift_package19_diagnostic','live_cloud_evidence_validation'}
...
def generate_receipt(bundle=None,run_audio_unit_host_probe=False,output=RECEIPT):
 ...
 validation={'suite_check':'pass','forbidden_runtime_resource_scan':'pass','retrieval_mirror_measurement':'pass','swift_package19_diagnostic':'pass','live_cloud_evidence_validation':'pass'}
...
No-tool completed **{no_tool['generation_samples']}** generation samples and **{no_tool['valid_supporting_judgments']}** valid supporting judgments in **{no_tool['generation_requests']}** provider requests, with no tools. Its artifact SHA-256 is `{no_tool['artifact_sha256']}`. Full-tool is honest partial evidence: **{full_tool['completed_generations']}** completed samples, six fallback-excluded samples, and four failed samples; only **{full_tool['valid_supporting_judgments']}** complete triplets reached valid supporting judgments in **{full_tool['generation_requests']}** provider requests. Its artifact SHA-256 is `{full_tool['artifact_sha256']}`. Both validators confirm the artifacts are outside live resource roots and generated summaries contain no assistant/judge prose, expected answers, credentials, hidden reasoning, or runtime-ineligible material.

No-tool's recorded P19 ledger snapshot spent **{no_tool['cost']['confirmed_p19_spent_microusd']} microUSD**; full-tool's later snapshot spent **{full_tool['cost']['confirmed_p19_spent_microusd']} microUSD**. Both preserve **{full_tool['cost']['reserved_microusd']} microUSD** reservations, including the pre-existing **{full_tool['cost']['external_unknown_hold_microusd']} microUSD** external unknown hold. The full-tool worktree patch hash is explicitly unavailable only for the immutable pre-fix artifact `{full_tool['artifact_sha256']}`; its commit and index-tree identities remain present. Future unavailable patch provenance is rejected. Ledger snapshots do not claim total provider cost outside the P19 ledger.
