...
def generate_receipt(bundle=None,run_audio_unit_host_probe=False,output=RECEIPT):
 ...
 validation={'suite_check':'pass','forbidden_runtime_resource_scan':'pass','retrieval_mirror_measurement':'pass','swift_package19_diagnostic':'pass','live_cloud_evidence_validation':'pass','live_cloud_evidence_tamper_regression':'pass'}
...
def reports(bundle=None):
 ...
 limitations=['Actual held-out retrieval quality misses the stated quality targets; no held-out tuning was performed.','Python and Swift neutral-contract metrics are required to reconcile exactly; report generation fails closed on an identity or metric mismatch.']+([] if long_context_complete else ['Long-context engine/store/tool coverage is not fully measured by the current provider-free diagnostic.'])+['No-tool completed 36 generation samples and 12 supporting judgments without tools. Full-tool is honest partial evidence: 26 completed, six fallback-excluded, and four failed samples; only five complete triplets reached a valid supporting judgment. Its worktree patch hash is recorded as unavailable solely for the pre-fix pipe deadlock; commit and index-tree identities are present, and future unavailable provenance is rejected.','Repeated triplets is honest partial evidence: 81 completed, 23 fallback-excluded (22 tool-round-limit engine failures and one timeout), and four malformed/no-usable-response failures; 18 of 36 triplets reached valid supporting judgments (8/7/3 by repetition).','The artifact cost snapshots preserve the $50 cap, retained reservations, and external unknown hold; they do not establish total provider cost outside the P19 ledger.','The provider-free long-context harness proves local engine/store/context/tool mechanics only; it does not measure cloud latency/cost, semantic model quality, or musician usefulness.','Only receipt-executed commands are marked pass; all other outcomes are unverified rather than inferred.']
...
No-tool's recorded P19 ledger snapshot spent **{no_tool['cost']['confirmed_p19_spent_microusd']} microUSD**; full-tool's later snapshot spent **{full_tool['cost']['confirmed_p19_spent_microusd']} microUSD**; repeated-triplets' later snapshot spent **{repeated['cost']['confirmed_p19_spent_microusd']} microUSD**. The latter preserves **{repeated['cost']['reserved_microusd']} microUSD** reservations, including the pre-existing **{repeated['cost']['external_unknown_hold_microusd']} microUSD** external unknown hold. The full-tool worktree patch hash is explicitly unavailable only for the immutable pre-fix artifact `{full_tool['artifact_sha256']}`; its commit and index-tree identities remain present. Future unavailable patch provenance is rejected. Repeated-triplets requires its exact clean-patch SHA-256. Ledger snapshots do not claim total provider cost outside the P19 ledger.
