...
def live_no_tool_evidence():
 """Fail closed on the approved, sanitized Lane A precompletion artifact.

 The returned projection deliberately contains only hashes, counts, statuses,
 and accounting totals. It never forwards request, response, or safe-error
 prose into generated reports.
 """
 try: evidence=json.loads(LIVE_NO_TOOL.read_text())
 except (OSError,json.JSONDecodeError) as error:raise ValueError('P19 live no-tool evidence missing or malformed') from error
 if evidence.get('schema_version')!='package019-cloud-evaluation/2' or evidence.get('package')!='019' or evidence.get('lane')!='no_tool':raise ValueError('P19 live no-tool schema/package/lane mismatch')
 expected_configuration={'provider':'openai-tutor-responses-v1','model':'gpt-5.6-sol','reasoning_effort':'high','service_tier':'priority','maximum_output_tokens':5000,'store':False,'cloud_text_consent':'explicit --cloud-text-consent','audio_present':False,'logic_context':'historical_p17_no_tool_context_source_vocal_plus_level_only'}
 if evidence.get('configuration')!=expected_configuration:raise ValueError('P19 live no-tool configuration mismatch')
 ...
 if not isinstance(hashes,dict) or any(hashes.get(key)!=value for key,value in expected_hashes.items()):raise ValueError('P19 live no-tool suite/index hash mismatch')
 prompt_hashes=hashes.get('cloud_prompt_suite',{})
 if not isinstance(prompt_hashes,dict) or prompt_hashes!={'exact_prompt_count':12,'ordered_prompt_suite_sha256':sh(ordered),'prompt_only_file_sha256':sh((Q/'package019_cloud_prompt_suite.json').read_bytes())}:raise ValueError('P19 live no-tool prompt hash mismatch')
 if hashes.get('policy')!=policy():raise ValueError('P19 live no-tool policy hash mismatch')
 source=evidence.get('source_identity',{})
 if not isinstance(source,dict) or not re.fullmatch(r'[0-9a-f]{40}',source.get('commit','')) or not re.fullmatch(r'[0-9a-f]{40}',source.get('index_tree','')) or not re.fullmatch(r'[0-9a-f]{64}',source.get('worktree_patch_sha256','')) or any('/' in str(value) for value in source.values()):raise ValueError('P19 live no-tool source identity is not path-free')
 counts=evidence.get('counts',{})
 expected_counts={'exact_prompt_count':12,'prompt_count':12,'levels':['noob','amateur','pro'],'level_count':3,'repetitions':1,'generation_samples':36,'provider_request_calls':36,'tool_enabled_samples':0,'supporting_judgments':12}
 if counts!=expected_counts:raise ValueError('P19 live no-tool count mismatch')
 attempts=evidence.get('attempts')
 if not isinstance(attempts,list) or len(attempts)!=36:raise ValueError('P19 live no-tool attempt count mismatch')
 prohibited_key_fragments=('authorization','credential','raw_request','request_body','request_headers','raw_response','response_body')
 def validate_value(value,path='root'):
  if isinstance(value,dict):
   for key,item in value.items():
    normalized=str(key).lower().replace('-','_')
    if any(fragment in normalized for fragment in prohibited_key_fragments):raise ValueError('P19 live no-tool evidence retained prohibited provider material')
    if normalized in {'assistant_text','judge_text','response_text'} and item not in ('',None):raise ValueError('P19 live no-tool evidence retained response text')
    validate_value(item,path+'.'+normalized)
  elif isinstance(value,list):
   for index,item in enumerate(value):validate_value(item,path+f'[{index}]')
 validate_value(evidence)
 empty_sha=sh(b'')
 for attempt in attempts:
  history=attempt.get('retry_history')
  if not isinstance(history,list) or len(history)!=1 or history[0].get('safe_category')!='provider_rejected':raise ValueError('P19 live no-tool unsafe failure category')
  if attempt.get('terminal_status')!='failed' or attempt.get('assistant_text')!='' or attempt.get('assistant_text_bytes')!=0 or attempt.get('assistant_text_sha256')!=empty_sha or any(attempt.get(key) is not None for key in ('provider_response_id','provider','model','service_tier','receipt_id')):raise ValueError('P19 live no-tool completion/metadata boundary mismatch')
  if attempt.get('provider_request_calls')!=1 or attempt.get('tools_enabled') is not False or attempt.get('tools_sent')!=[] or attempt.get('tool_calls')!=[] or attempt.get('tool_results')!=[] or attempt.get('tool_receipts')!=[] or attempt.get('tool_rounds')!=0:raise ValueError('P19 live no-tool request/tool count mismatch')
 triplets=evidence.get('deterministic_triplet_summaries')
 judgments=evidence.get('supporting_model_assisted_judgments')
 if not isinstance(triplets,list) or len(triplets)!=12 or not all(item.get('generation_complete_before_reference') is False and item.get('semantic_key_acceptable_family_check',{}).get('reference_read') is False for item in triplets if isinstance(item,dict)):raise ValueError('P19 live no-tool reference firewall mismatch')
 if not isinstance(judgments,list) or len(judgments)!=12 or not all(item.get('status')=='skipped_incomplete_generation' and item.get('semantic_reference_read') is False and item.get('provider_request_calls',0)==0 for item in judgments if isinstance(item,dict)):raise ValueError('P19 live no-tool supporting-judge boundary mismatch')
 cost=evidence.get('cost',{})
 expected_cost={'schema_version':'tracksmith-cloud-evaluation-budget/v1','cap_microusd':50000000,'external_unknown_hold_microusd':9888608,'reserved_microusd':18105512,'spent_microusd':0,'reservation_count':37,'poisoned':False,'pricing_source':'openai-gpt-5.6-sol-fast-2026-08-25'}
 if cost!=expected_cost or cost['reserved_microusd']>cost['cap_microusd'] or cost['reserved_microusd']!=cost['external_unknown_hold_microusd']+(cost['reserved_microusd']-cost['external_unknown_hold_microusd']):raise ValueError('P19 live no-tool cost accounting mismatch')
 if evidence.get('terminal_failure_sample_count')!=36 or evidence.get('failure_count')!=36 or evidence.get('global_budget_failure_fail_fast') is not False:raise ValueError('P19 live no-tool terminal accounting mismatch')
 if LIVE_NO_TOOL.is_relative_to(ROOT/'packages/ProductionTutor/Sources/ProductionTutor/Resources') or LIVE_NO_TOOL.is_relative_to(ROOT/'packages/TutorConversation/Sources/TutorConversation/Resources'):raise ValueError('P19 live no-tool evidence entered a runtime resource root')
 return {'status':'accepted_attempted_failed_precompletion_provider_rejected','artifact_sha256':sh(LIVE_NO_TOOL.read_bytes()),'artifact_bytes':LIVE_NO_TOOL.stat().st_size,'schema_version':evidence['schema_version'],'lane':'no_tool','generation_requests':36,'terminal_failed_generation_samples':36,'completed_generation_samples':0,'assistant_text_bytes':0,'provider_response_metadata_count':0,'supporting_judge_provider_requests':0,'cost':{'cap_microusd':cost['cap_microusd'],'external_unknown_hold_microusd':cost['external_unknown_hold_microusd'],'reserved_microusd':cost['reserved_microusd'],'confirmed_p19_spent_microusd':cost['spent_microusd'],'poisoned':cost['poisoned']},'runtime_resource_exclusion':'passed_artifact_outside_live_resource_roots'}
...
ARTIFACT_PATHS={'suite':SUITE,'manifest':MANIFEST,'candidate_index':INDEX,'candidate_manifest':IM,'policy':POLICY,'tool_diagnostic':E/'PACKAGE_019_TOOL_DIAGNOSTIC.json','live_no_tool_evidence':LIVE_NO_TOOL}
REQUIRED_VALIDATION_KEYS={'suite_check','forbidden_runtime_resource_scan','retrieval_mirror_measurement','swift_package19_diagnostic','live_no_tool_evidence_validation'}
...
def valid_receipt(path=RECEIPT,bundle=None):
 ...
 if receipt.get('artifact_hashes')!=expected_hashes or receipt.get('runtime_provenance')!=runtime_provenance():return None
...
def generate_receipt(bundle=None,run_audio_unit_host_probe=False,output=RECEIPT):
 ...
 validation={'suite_check':'pass','forbidden_runtime_resource_scan':'pass','retrieval_mirror_measurement':'pass','swift_package19_diagnostic':'pass','live_no_tool_evidence_validation':'pass'}
 ...
 receipt={'schema':'package019.validation-receipt/v1','candidate_fingerprint':candidate_fingerprint(),'artifact_hashes':artifact_hashes(bundle),'runtime_provenance':runtime_provenance(),'validation':validation,'boundary':'Only listed commands were executed by this receipt generator. When present, a built-bundle scan is bound to its deterministic inventory hash. No signing, install, auval, Logic, audio, cloud, model-assisted, or owner evidence is inferred.'}
 put(output,receipt);return receipt

def receipt_self_test():
 required_validation={key:'pass' for key in REQUIRED_VALIDATION_KEYS};artifacts=artifact_hashes();valid={'schema':'package019.validation-receipt/v1','candidate_fingerprint':candidate_fingerprint(),'artifact_hashes':artifacts,'runtime_provenance':runtime_provenance(),'validation':required_validation}
 ...
 with tempfile.TemporaryDirectory(prefix='p19-receipt-') as temporary:
  ...
  bundled={'schema':'package019.validation-receipt/v1','candidate_fingerprint':candidate_fingerprint(),'artifact_hashes':artifact_hashes(bundle),'runtime_provenance':runtime_provenance(),'validation':dict(required_validation,installed_bundle_resource_scan='pass')};put(path,bundled)
...
def reports(bundle=None):
 ...
 lanes={'cloud_no_tool':'attempted_failed_precompletion_provider_rejected (36/36)','cloud_full_tools':'not_run_after_same_provider_path_blocked_and_reservations_reduced_safe_headroom','cloud_repeated_triplets':'not_run_after_same_provider_path_blocked_and_reservations_reduced_safe_headroom','audio':'not_run_no_explicit_cloud_audio_consent','Logic':'not_run_no_authorized_Logic_session','owner_listening':'not_run_no_owner_listening','signed_install':'not_run_not_receipted','installed_AU_registration_auval':'not_run_not_receipted','unsigned_CompanionMacApp_build':commands['unsigned_Release_CompanionMacApp_build'],'unsigned_AU_source_build':commands['unsigned_Release_AssistantAudioUnitExtension_build'],'AudioUnitHostProbe':commands['AudioUnitHostProbe']}
 ...
 unmet=['held-out top-1, top-4, no-match precision, and no-match recall targets missed','ambiguity is model-withheld because its held-out calibration is below 0.75']+([] if long_context_complete else ['long-context engine/store/tool coverage is not fully measured by the current provider-free diagnostic'])+['signed/install/AU registration/auval, Logic, audio, full-tool cloud, repeated cloud, model-assisted, and owner lanes not run','remote CI pending']+([] if receipt else ['machine validation receipt missing or mismatched; command outcomes are unverified'])
 limitations=['Actual held-out retrieval quality misses the stated quality targets; no held-out tuning was performed.','Python and Swift neutral-contract metrics are required to reconcile exactly; report generation fails closed on an identity or metric mismatch.']+([] if long_context_complete else ['Long-context engine/store/tool coverage is not fully measured by the current provider-free diagnostic.'])+['The consented no-tool cloud attempt ended before any completed response. Confirmed P19 ledger spend is 0 microUSD, while all reservations remain retained and the earlier external unknown hold remains an accounting boundary; this does not establish total provider cost outside that ledger.','The provider-free long-context harness proves local engine/store/context/tool mechanics only; it does not measure cloud latency/cost, semantic model quality, or musician usefulness.','Only receipt-executed commands are marked pass; all other outcomes are unverified rather than inferred.']
...
The consented no-tool Lane A attempt made **{live['generation_requests']}** generation requests and ended before completion at all **{live['terminal_failed_generation_samples']}** samples with `provider_rejected`; it retained zero assistant-text bytes, zero provider response metadata, and zero supporting-judge provider requests. Its artifact SHA-256 is `{live['artifact_sha256']}` and the validator confirms it is outside live resource roots. Confirmed P19 ledger spend is **{live['cost']['confirmed_p19_spent_microusd']} microUSD**; **{live['cost']['reserved_microusd']} microUSD** stays reserved, including the pre-existing **{live['cost']['external_unknown_hold_microusd']} microUSD** external unknown hold. This ledger statement does not claim total provider cost outside the P19 ledger.
