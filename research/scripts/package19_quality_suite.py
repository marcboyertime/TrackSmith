...
No-tool's recorded P19 ledger snapshot is **{no_tool['cost']['ledger_snapshot_spent_microusd']} microUSD**; full-tool's later snapshot is **{full_tool['cost']['ledger_snapshot_spent_microusd']} microUSD**; repeated-triplets' later snapshot is **{repeated['cost']['ledger_snapshot_spent_microusd']} microUSD**. These immutable historical artifacts lack cache-classified provider usage detail, so none is an exact confirmed provider-spend claim. The latter preserves **{repeated['cost']['reserved_microusd']} microUSD** reservations, including the pre-existing **{repeated['cost']['external_unknown_hold_microusd']} microUSD** external unknown hold. The full-tool worktree patch hash is explicitly unavailable only for the immutable pre-fix artifact `{full_tool['artifact_sha256']}`; its commit and index-tree identities remain present. Future unavailable patch provenance is rejected. Repeated-triplets requires its exact clean-patch SHA-256. A new consented run under cache-classified accounting is required for an exact provider-spend result.
