...
The indexed path mirrors CandidateRetrievalIndex: metadata-only FTS selection,
general structured rerank, question-key dedupe and early domain diversity, then
bounded payload reads. The legacy path reads research-owned JSON only as a
migration oracle; neither path writes product resources.
"""
from __future__ import annotations

import argparse, hashlib, json, math, re, sqlite3, statistics, subprocess, sys, time
...
def tokens(text: str) -> list[str]: return sorted({token for raw in re.findall(r"[a-z0-9]+", text.lower().replace("_", " ")) if (token := canonical(raw))})
...
def indexed(con: sqlite3.Connection, text: str, details: bool = False) -> dict:
    ...
    query, min_overlap = set(ts), max(2, math.ceil(len(ts) * MINIMUM_COVERAGE))
    ...
    selected, questions, domain_counts, package_counts = [], set(), {}, {}
    for item in ordered:
        if item["question_key"] in questions or domain_counts.get(item["domain"], 0) >= 2 or package_counts.get(item["package_id"], 0) >= 2: continue
        questions.add(item["question_key"]); domain_counts[item["domain"]] = domain_counts.get(item["domain"], 0) + 1; package_counts[item["package_id"]] = package_counts.get(item["package_id"], 0) + 1; selected.append(item)
...
def legacy_rank(cards: list[dict], text: str) -> list[str]:
    query, rows = set(tokens(text)), []
    for card in cards:
        primary = set(tokens(" ".join(str(card.get(key, "") or "") for key in ("title", "question", "domain", "category", "topic"))))
        contextual = set(tokens(" ".join(str(value) for key in ("tags", "clarificationQuestions", "competingHypotheses", "listeningCues") for value in (card.get(key, []) if isinstance(card.get(key, []), list) else [card.get(key, "")]))))
