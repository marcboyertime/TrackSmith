...
(`com.apple.logic10`, universal binary), Swift 6.2.1, Command Line Tools only.
...
|---|---|---|---|---|
| AUv3 effect | Reads its input buses and host callbacks; writes its output and own state/parameters | No selected-region API; input-stream audio only; controls own parameters; host may automate them | Supported on Apple Silicon; extension sandbox; no microphone permission for insert audio | DSP/extension source and generated Xcode project exist. Logic build/validation pending full Xcode. **Production core.** |
...
| Native companion | Owns UI, disk, models, preview cache, Keychain, imports | Cannot introspect Logic merely by being installed | Sandboxed; user-selected file access; network only when enabled | CLI slice builds; SwiftUI target generated but not compiled. **Production.** |
...
| Buffer input in plug-in | Copies audio already passing through insert into bounded memory | Bounded recent/next interval; not the selected source file | No microphone permission; no disk/file work on render | Lock-free ring and chronology test pass. Apple says AU extensions cannot perform recording, so disk recording is excluded; this is an in-memory analysis copy. **Production after host test.** |
...
6. **Can it buffer a bounded interval?** Technically yes for incoming samples with a preallocated ring; no file I/O occurs on render. Logic validation remains open.
