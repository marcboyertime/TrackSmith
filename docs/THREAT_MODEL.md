...
| retrieved source → answer | outdated or superseded source, low-quality source dominating, one tutorial monopolizing an answer, caption mis-transcription of a plug-in name | typed source registry with supersession and tier; per-source diversity cap in retrieval; audiovisual-review gate blocks transcript-only grounding of claims that depend on hearing or seeing |
| model → answer prose | invented citation, invented procedure ID, uncited numeric setting, claimed action or hearing, undisclosed contradiction, personal result generalized | answer validator resolves every ID locally, requires numbers to be quoted from cited reviewed cards or carried by a validated procedure, scans forbidden claims, requires contradiction disclosure, rejects universalized personal results |
| research path → trusted knowledge | provisional result silently promoted; exact steps invented from a web page | research results stay labeled provisional, cannot create an exact procedure, and enter only a review queue as candidate cards |
| measurement → user | audio presented as informing an answer it did not influence | MeasurementRelevanceMap records the metrics that actually influenced the answer; validator rejects claimed influence with no influencing metric |
