...
`plan_work.py` and `ci/semantic_dependencies.json` provide rename/deletion-aware advisory planning for local use and GitHub summaries. Main/manual validation and unknown/shared changes are full; required remote workflows deliberately remain always-run. `deterministic_case_cache.py` is local-only and unused by CI. Its enforced schema accepts only suite/version/case identifiers plus source, policy, index, toolchain, and semantic hashes, with a passed-case result hash; unknown fields, prose/blob payloads, provider-response-shaped fields, credentials, failures, traversal/symlink roots, and entries are rejected. Pruning never initializes a cache: it needs a descriptor-safe ownership index and retires only indexed entries whose compact payload validates against its filename key; pathname replacement is never deleted. No GitHub cache is enabled.
