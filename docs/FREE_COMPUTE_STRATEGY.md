...
For a future Codex/package change, first assign deterministic Python/corpus/index/report work to one existing Linux lane; assign Swift tests to `macos_swift_core` or `macos_tutor`; and assign unsigned Xcode resource checks to the combined Xcode lane. Add a manifest row before adding a job. Keep provider, cloud, private audio, signing/install, Logic, and listening work owner-controlled and absent from Actions. The fail-closed audit parses every workflow, rejects PATH/shell command-resolution mutation, binds required jobs to their exact manifest commands, recursively discovers repository-local shell and Python helpers, and rejects unresolved/dynamic helper paths. Xcode build-like invocations must use literal `CODE_SIGNING_ALLOWED=NO`; an artifact upload must name one exact manifest output sealed from verified regular inputs into a bounded snapshot in the immediately preceding guard. Checked-in audit exceptions are forbidden because a pull request could otherwise authorize its own bypass. These are deterministic in-repository guardrails, not an independent defense against a malicious pull request that can edit the audit itself; external required-check governance supplies that separate trust boundary. Do not add caches until a measured benefit justifies their documented budget.
...
The Tutor lane builds `TutorConversationTests` once and invokes the release binary in a bounded number of local shards. It defaults to reserving zero logical CPUs, based on the measured three-way hosted run; owners can set `TRACKSMITH_TUTOR_RESERVED_CPUS=1` or higher. The executable cap is always three Tutor workers and never exceeds detected logical CPUs. `TRACKSMITH_TUTOR_SHARDS` can request fewer workers only. Each ordinary case has a stable identifier, a resource class, and a cost-balanced LPT assignment. The exact versioned `ci/tutor_test_costs.json` manifest binds the suite, source/policy/index hashes, and complete case universe; the runner validates that universe against the release binary before execution. `aggregate_shards.py` fails closed unless all validated-manifest cases execute exactly once with matching suite, source, commit/tree classification, toolchain, policy, index, algorithm, and result hashes. A no-argument run remains the complete 36-test historical suite.

`plan_work.py` and `ci/semantic_dependencies.json` provide rename/deletion-aware local execution planning and GitHub summaries. `bash scripts/ci/run-burst-local.sh --base BASE_SHA` consumes its bounded manifest commands; main/manual/merge preparation and unknown/shared paths are full. Required remote workflows deliberately remain always-run. `deterministic_case_cache.py` is a local-only, opt-in execution layer used by `TRACKSMITH_TUTOR_CASE_CACHE_DIR=PATH bash scripts/ci/run-macos-tutor.sh`; it never uploads, transports, or reuses a failure. Its enforced schema accepts only suite/version/case identifiers plus shared-evaluator, policy, index, toolchain, and semantic hashes, with a passed-case result hash; unknown fields, prose/blob payloads, provider-response-shaped fields, credentials, failures, traversal/symlink roots, and entries are rejected. The shared evaluator is a complete deterministic execution-source closure: redacted Tutor harness code plus `Package.swift`, package sources/resources, Tutor resources, the Package 15 community-knowledge package tree, the exact product configuration/app source files directly read by ordinary cases, and the P16 runtime projection. The cache keys each ordinary body separately while this closure, policy, index, or toolchain change invalidates every affected key. The binary SHA-256 is recorded as observation, not a key, because it would erase legitimate one-case reuse after a source-only rebuild. Pruning never initializes a cache: it needs a descriptor-safe ownership index and retires only indexed entries whose compact payload validates against its filename key; pathname replacement is never deleted. No GitHub cache is enabled.
