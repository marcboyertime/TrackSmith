...
For a future Codex/package change, first assign deterministic Python/corpus/index/report work to one existing Linux lane; assign Swift tests to `macos_swift_core` or `macos_tutor`; and assign unsigned Xcode resource checks to the combined Xcode lane. Add a manifest row before adding a job. Keep provider, cloud, private audio, signing/install, Logic, and listening work owner-controlled and absent from Actions. The fail-closed audit parses every workflow, binds required jobs to their exact manifest commands, recursively discovers repository-local shell and Python helpers, and rejects unresolved/dynamic helper paths. Xcode build-like invocations must use literal `CODE_SIGNING_ALLOWED=NO`; artifact uploads must name exact manifest files and have a preceding 25 MB size guard. Checked-in audit exceptions are forbidden because a pull request could otherwise authorize its own bypass. Do not add caches until a measured benefit justifies their documented budget.
